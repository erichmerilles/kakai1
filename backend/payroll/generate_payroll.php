...
try {
  ...
  foreach ($employees as $e) {
    ...
    $hourly_base = (float)$e['daily_rate'] / 8;
    ...
    if ($gross_pay > 0) {
      ...
      $caStmt = $pdo->prepare("SELECT ca_id, balance FROM cash_advance WHERE employee_id = ? AND status = 'Approved' AND is_paid = 0 ORDER BY created_at ASC");
      ...
      foreach ($cashRows as $c) {
        ...
        if ($available_pay >= $current_balance) {
          ...
          $ca_updates[] = ['new_balance' => 0, 'is_paid' => 1, 'ca_id' => $ca_id];
        } else {
          $total_deducted += $available_pay;
          $ca_updates[] = ['new_balance' => $current_balance - $available_pay, 'is_paid' => 0, 'ca_id' => $ca_id];
          $available_pay = 0;
        }
      }
      ...
      if (!empty($ca_updates)) {
        $updCA = $pdo->prepare("UPDATE cash_advance SET balance = ?, is_paid = ?, payroll_id = IF(?=1, ?, payroll_id) WHERE ca_id = ?");
        foreach ($ca_updates as $upd) {
          $updCA->execute([$upd['new_balance'], $upd['is_paid'], $upd['is_paid'], $payroll_id, $upd['ca_id']]);
        }
      }
